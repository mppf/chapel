...
/* A record representing a date */
  ...
  */
  ...
  proc date.strftime(fmt: string) {
    ...
    strftime(c_ptrTo(buf), bufLen, c_ptrToConst_helper(fmt):c_string, timeStruct);
    var str: string;
    try! {
  ...
  proc time.strftime(fmt: string) {
    ...
    if timezone.borrow() != nil {
      ...
      timeStruct.tm_zone = __primitive("cast", tm_zoneType, c_ptrToConst_helper(tzname()):c_string);
      timeStruct.tm_isdst = dst().seconds: int(32);
    } else {
      timeStruct.tm_gmtoff = 0;
      timeStruct.tm_zone = __primitive("cast", tm_zoneType, c_ptrToConst_helper(""):c_string);
  ...
  */
  ...
  proc type dateTime.strptime(date_string: string, format: string) {
    ...
    var timeStruct: tm;
    strptime(c_ptrToConst_helper(date_string):c_string, c_ptrToConst_helper(format):c_string, timeStruct);
  ...
  proc dateTime.strftime(fmt: string) {
    ...
    strftime(c_ptrTo(buf), bufLen, c_ptrToConst_helper("".join(strftok(fmt))):c_string, timeStruct);
