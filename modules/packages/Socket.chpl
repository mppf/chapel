...
proc sys_sockaddr_t.init(in other: sys_sockaddr_t) {
  ...
  try! {
    ...
    this.set(c_ptrToConst_helper(host):c_string, port, family:c_int);
  }
}
...
proc type ipAddr.create(host: string = "127.0.0.1", port: uint(16) = 8000,
  ...
  addressStorage.set(c_ptrToConst_helper(host):c_string, port, family:c_int);
...
proc connect(in host: string, in service: string, family: IPFamily = IPFamily.IPUnspec,
  ...
  var err = getaddrinfo(c_ptrToConst_helper(host):c_string, c_ptrToConst_helper(service):c_string, hints, result);
  if err != 0 {
...
proc udpSocket.send(data: bytes, in address: ipAddr,
  ...
  var err_out:c_int = 0;
  var length:c_ssize_t;
  err_out = sys_sendto(this.socketFd, c_ptrToConst_helper(data):c_string:c_ptr(void), data.size:c_long, 0, address._addressStorage, length);
  if err_out == 0 {
    return length;
  }
  if err_out != 0 && err_out != EAGAIN && err_out != EWOULDBLOCK {
...
proc setSockOpt(socketFd:c_int, level: c_int, optname: c_int, ref value: bytes) throws {
  ...
  var ptroptval = c_ptrToConst_helper(value):c_string;
