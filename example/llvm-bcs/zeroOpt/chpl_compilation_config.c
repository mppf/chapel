...
#include <stdio.h>
const char* chpl_compileCommand = "chpl --baseline -g -o example/experiment example/experiment.chpl --llvm --savec=example/llvm-bcs/zeroOpt/ ";
const char* chpl_compileVersion = "1.10.0.ed08a35";
...
void chpl_program_about() {
  printf("%s", "Compilation command: chpl --baseline -g -o example/experiment example/experiment.chpl --llvm --savec=example/llvm-bcs/zeroOpt/ \n");
  printf("%s", "Chapel compiler version: 1.10.0.ed08a35\n");
