            ...
            for or in [true, false] {
              ...
              for ow in [true, false] {
                ...
                for ox in [true, false] {
                  ...
                  err = chmod("blah".c_str(), perm: mode_t);
                  if err != 0 then halt("error in chmod");
                  err = stat("blah".c_str(), c_ptrTo(structStat));
...
permission = 448;
var perm = permission:c_int;
err = chmod("blah".c_str(), perm: mode_t); // reset blah's permissions to only include user permissions
if err != 0 then halt("error in chmod");
