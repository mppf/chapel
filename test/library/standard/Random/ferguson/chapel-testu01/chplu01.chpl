...
if testReal {
  ...
  rs = getRNG();
  if crush then
    run_crush_testu01_double(localName.c_str());
  else
    run_smallcrush_testu01_double(localName.c_str());
} else {
  ...
  rs = getRNG();
  if crush then
    run_crush_testu01_uint(localName.c_str());
  else
    run_smallcrush_testu01_uint(localName.c_str());
}
