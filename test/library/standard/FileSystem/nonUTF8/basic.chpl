...
proc getMode(filename: string) throws {
  ...
  var err = stat(c_ptrToConst_helper(filename.encode(policy=encodePolicy.unescape)), c_ptrTo(structStat));
...
var err = chmod(c_ptrToConst_helper(filename2.encode(policy=encodePolicy.unescape)),
                newMode:mode_t);
if err != 0 then halt("Error in chmod call: ", strerror(errno));
writeln("chmod works: ", getMode(filename2) == newMode);
err = chmod(c_ptrToConst_helper(filename2.encode(policy=encodePolicy.unescape)),
