...
proc main {
  ...
  /* Get the varids of the latitude and longitude coordinate
     variables. */
  cdfError(nc_inq_varid(ncid, latName.c_str(), lat_varid));
  cdfError(nc_inq_varid(ncid, lonName.c_str(), lon_varid));
  ...
  /* Get the varids of the pressure and temperature netCDF
     variables. */
  cdfError(nc_inq_varid(ncid, presName.c_str(), pres_varid));
  cdfError(nc_inq_varid(ncid, tempName.c_str(), temp_varid));
  ...
  /* Each of the netCDF variables has a "units" attribute. Let's read
     them and check them. */
  cdfError(nc_get_att_text_void_ptr(ncid, lat_varid, units.c_str(), latUnitsIn.ptr()));
  if latUnitsIn.ptr(): c_string: string != latUnits {
    writeln("Bad units for latUnits");
    return 2;
  }

  cdfError(nc_get_att_text_void_ptr(ncid, lon_varid, units.c_str(), lonUnitsIn.ptr()));
  if lonUnitsIn.ptr(): c_string: string != lonUnits {
    writeln("Bad units for lonUnits");
    return 2;
  }

  cdfError(nc_get_att_text_void_ptr(ncid, pres_varid, units.c_str(), presUnitsIn.ptr()));
  if presUnitsIn.ptr(): c_string: string != presUnits {
    writeln("Bad units for presUnits");
    return 2;
  }
  cdfError(nc_get_att_text_void_ptr(ncid, temp_varid, units.c_str(), tempUnitsIn.ptr()));
