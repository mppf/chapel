...
sync coforall loc in Locales { // look at this more
  ...
  on loc {
    ...
    var hdfsFS: c_ptr(void) = HDFS.hdfsConnect(c_ptrToConst_helper(namenode.localize()):c_string,
                                              connectNumber);
    writeln("done");
    var fileInfo = HDFS.chadoopGetFileInfo(hdfsFS, c_ptrToConst_helper(dataFile.localize()):c_string);
    //      writeln("Info for file " + dataFile + ":");
    //      writeln(fileInfo);
    var blockHosts = HDFS.hdfsGetHosts(hdfsFS, c_ptrToConst_helper(dataFile.localize()):c_string, 0, fileInfo.mSize);
    // ==== Get the blocks
    var blockCount = HDFS.chadoopGetBlockCount(blockHosts);
    // END
    ...
    var dataFileLocal = HDFS.hdfsOpenFile(hdfsFS, c_ptrToConst_helper(dataFile.localize()):c_string,
                                          O_RDONLY, 0, 0, 0);
